...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const [cats, prods] = await Promise.all([
        ...
        client.fetch(`*[_type == "product"]{ _id, name, priceNGN, images, _createdAt }`)
      ]);
  ...
  return (
    <>
      <nav className={`fixed ${isHomePage ? "top-8" : "top-0"} z-[100] w-full transition-all duration-700 ${navBg}`}>
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 relative">
                ...
                <div className="bg-white border shadow-xl py-3 min-w-[120px]">
                  {(["NGN", "USD", "GBP", "EUR", "CAD"] as const).map((cur) => (
                    <button
                      ...
                      className={`block w-full text-left px-6 py-2 text-[9px] ${
                        currency === cur
          ...
          <button onClick={() => setIsOpen(true)} className={`p-2 lg:hidden ${iconColor}`}>
            <Menu size={28} />
          </button>
          ...
          <div className="flex items-center gap-3 sm:gap-6">
            ...
            <button onClick={() => setIsSearchOpen(true)} className={iconColor}>
              <Search size={22} />
            </button>
            ...
            <button onClick={() => setIsCartOpen(true)} className={`relative ${iconColor}`}>
              <ShoppingBag size={22} />
      ...
      {isOpen && (
        <div className="fixed inset-0 bg-black/5 z-[120]" onClick={() => setIsOpen(false)} />
      )}
